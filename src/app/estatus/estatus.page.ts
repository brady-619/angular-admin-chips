...
export class EstatusPage implements OnInit {
  ...
  constructor(private http: HttpClient, private updatestatus: UpdateStatusService,public alertCtrl: AlertController) { }
  ...
  getPayload(data: any) {
    ...
    for (let i of data) {
      ...
      if (items[0] == undefined || items[1] == undefined) {
        ...
        tmp.push({ ICCI: items[0], ESTATUS: items[1] })
      }
  ...
  async createUsersMulti() {
    ...
    await this.updatestatus.UpdateStatus(data2).then(async respuesta => {
      console.log(data2)
